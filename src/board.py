...
class Board:
    ...
    def is_check(self, color):
        ...
        for row in range(ROWS):
            for col in range(COLS):
                if self.squares[row][col].has_team_piece(color) and isinstance(self.squares[row][col].piece, King):
                    king_pos = (row, col)
                    break
            if king_pos: break
    ...
    def calc_moves(self, piece, row, col, bool=True):
        if piece is None:  # Adiciona verificação para peça existente
            return
    ...
    def _add_pieces(self, color):
        ...
        for col, piece in enumerate(pieces):
            self.squares[row_other][col] = Square(row_other, col, piece)
